...
La résolution `.lnk` utilise uniquement l’interop COM typée `IPersistFile` et `IShellLinkW`, avec des drapeaux sans interface, recherche, suivi ou mise à jour. PersistLens ne lance ni le raccourci, ni sa cible, ni un shell. Tous les champs résolus sont non fiables ; une cible relative est normalisée au mieux depuis le dossier de travail ou le dossier du raccourci et une cible inexistante reste distincte.

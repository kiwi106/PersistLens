...
public static class PersistLensProgram
{
    ...
    private static async Task<int> SnapshotAsync(Arguments arguments, InventoryService service, OperatingContext context, ISnapshotStore store, IReporter reporter, TextWriter output, CancellationToken token)
    {
        ...
        if (action == "list") { if (arguments.Redact) return await InvalidAsync(output, "--redact s’applique uniquement aux rapports inventory, inspect, snapshot show et diff.").ConfigureAwait(false); foreach (var snapshotName in await store.ListAsync(token).ConfigureAwait(false)) await output.WriteLineAsync(snapshotName).ConfigureAwait(false); return 0; }
        ...
        if (action == "delete") { if (arguments.Redact) return await InvalidAsync(output, "--redact s’applique uniquement aux rapports inventory, inspect, snapshot show et diff.").ConfigureAwait(false); await store.DeleteAsync(name, token).ConfigureAwait(false); await output.WriteLineAsync($"Snapshot « {name} » supprimé.").ConfigureAwait(false); return 0; }
    ...
    {
        var storage = default(string); var json = false; var redact = false; var remaining = new List<string>(); var help = false; var version = false;
        ...
        return new(command, positionals, storage, json, redact, help, version);
    }
    private sealed record Arguments(string Command, IReadOnlyList<string> Positionals, string? StorageDirectory, bool Json, bool Redact, bool ShowHelp, bool ShowVersion);
    private const string Help = "PersistLens - inventaire local des mécanismes de persistance Windows\n\nCommandes :\n  persistlens inventory [--format json] [--redact] [--storage DOSSIER]\n  persistlens snapshot create --name NOM [--storage DOSSIER]\n  persistlens snapshot list [--storage DOSSIER]\n  persistlens snapshot show NOM [--format json] [--redact] [--storage DOSSIER]\n  persistlens snapshot delete NOM [--storage DOSSIER]\n  persistlens diff SNAPSHOT-A SNAPSHOT-B|current [--format json] [--redact] [--storage DOSSIER]\n  persistlens inspect ENTRY-ID [--format json] [--redact]\n\n--redact produit un rapport partageable avec masquage best effort des données sensibles connues. Il ne garantit pas la suppression de tout secret ; le rapport complet reste préférable pour l’analyse locale. Les snapshots originaux ne sont jamais modifiés.\n\nCodes de sortie : 0 succès ; 1 différences détectées ; 2 entrée invalide ; 3 erreur opérationnelle ; 4 collecte partielle.";
}
